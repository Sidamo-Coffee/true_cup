...
| スキル | 何の手順か |
| --- | --- |
| `pr-review` | PR をサブエージェントにレビューさせる（ユーザーが明示的に実行したときのみ） |
...
4. **ビューやスタイルを変更したなら、スマートフォン幅で表示を確認する**（`mobile-check` スキル）。
   このアプリは主にスマートフォンから使われるため、モバイル幅の崩れは実使用に直撃する。
   Tailwind のクラスから机上で計算しても崩れは見つけられず、実際に描画させないと分からない
5. PR を作成しタイトルに Issue 番号を含める
...
**PR レビューはユーザーが明示的に依頼した場合のみ行う。通常1回、重大指摘の修正時のみ再レビュー1回、
合計最大2回。** PR を作成しただけでは起動しない。

指摘は鵜呑みにせず自分で再現を確認したうえで妥当性を判断し、対応した内容と
**取り込まなかった指摘とその理由**を PR 本文に残す。

**レビュー結果をそのまま PR コメントに投稿しない。** 判断を経ていない指摘が公開記録に残るのを避ける。

手順は `pr-review` スキルにある。
