...
レビューは最後の網でしかない（公開はPR作成の時点で成立し、事後の編集では回収できない）。

Agent ツール（`general-purpose`）にシニアエンジニアとして振る舞うよう指示し、
PR 番号を渡して差分と本文を確認させる。レビューさせる観点:

- 公開すべきでない情報が PR 本文・差分に含まれていないか（上記参照）
- PR 説明の記述が実際の変更と食い違っていないか（件数・バージョン・挙動の説明など）
- 変更範囲が Issue のスコープを超えていないか、逆に必要な変更が漏れていないか
- テストが変更した挙動を実際に検証しているか。
  CI でも RSpec は実行されるが、緑であることと「変更した挙動を検証できていること」は別。
  削除しても spec が通る分岐が残っていないか（変異テスト）まで見させると精度が上がる

**実装側の責務として**、指摘は鵜呑みにせず妥当性を判断し、
