...
6. **作成した PR をサブエージェントにレビューさせる**（`pr-review` スキル）
7. CI 通過後にマージ（マージはユーザーが行う）
...
**実装側の責務として**、指摘は鵜呑みにせず自分で再現を確認したうえで妥当性を判断する。
対応した内容と、**取り込まなかった指摘とその理由**を PR 本文に残す。判断の経緯を後から追えるようにするため。
